...
from .eval_snapshot_n import meta_test, meta_val
sys.path.append('..')
...
def get_opt(model,args):
    ...
    scheduler = optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=args.epoch)
...
class Train_Manager:
    ...
    def train(self,model):
        ...
        for e in tqdm(range(total_epoch)):
            ...
            if (e+1)%args.epoch==0:
                ...
                torch.save(model.state_dict(),str(e+1)+'_'+save_path)
                logger.info(str(e+1)+' Save!')
    ...
    def evaluate(self,model):
        ...
        with torch.no_grad():
            ...
            models = [copy.deepcopy(model) for i in range(args.stage)]

            [models[i].load_state_dict(torch.load(str(args.epoch*(i+1))+'_'+self.save_path)) for i in range(args.stage)]
